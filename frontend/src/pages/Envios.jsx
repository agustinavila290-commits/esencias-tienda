...
function Seccion({ icono, titulo, children }) {
  return (
    <div className="bg-surface rounded-2xl p-6 border border-brand-primary-100 shadow-sm">
      <div className="flex items-center gap-3 mb-4">
        {typeof icono === 'string'
          ? <span className="text-2xl">{icono}</span>
          : icono}
        <h2 className="font-display text-xl font-bold text-brand-primary-800">{titulo}</h2>
      </div>
      {children}
    </div>
  )
...
export default function Envios() {
  return (
    <div className="max-w-2xl mx-auto px-4 py-8 pb-10">
      ...
      <h1 className="font-display text-3xl font-bold text-brand-primary-800 mb-6">Envíos y medios de pago</h1>
      ...
      <div className="space-y-5">
          ...
          <div className="space-y-3 text-sm text-text-secondary leading-relaxed">
            ...
            <div className="space-y-2">
              {[
                ...
                <div key={label} className="flex items-start gap-2 bg-brand-primary-50 rounded-xl p-3">
                  <span className="text-brand-primary-500 font-semibold flex-shrink-0 mt-0.5">→</span>
                  <div>
                    <p className="font-medium text-text-primary">{label}</p>
                    <p className="text-text-secondary text-xs">{detalle}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="space-y-3 text-sm text-text-secondary">
            <div className="grid grid-cols-1 gap-2">
              {[
                ...
                <div key={name} className="flex items-start gap-3 bg-brand-primary-50 rounded-xl p-3">
                  <span className="text-xl flex-shrink-0">{icon}</span>
                  <div>
                    <p className="font-semibold text-text-primary">{name}</p>
                    <p className="text-text-secondary text-xs leading-relaxed">{detalle}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <Seccion icono="↩️" titulo="Cambios y devoluciones">
          <div className="text-sm text-text-secondary leading-relaxed space-y-2">
            ...
            <p className="text-xs text-text-secondary">Por la naturaleza artesanal de los productos, no se aceptan cambios por preferencia de aroma.</p>
          </div>
        </Seccion>

        <div className="bg-gradient-to-br from-brand-primary-700 to-brand-primary-600 rounded-2xl p-6 text-white text-center">
          <p className="font-semibold mb-1">¿Tenés alguna duda?</p>
          <p className="text-brand-primary-200 text-sm mb-4">Escribinos y te respondemos a la brevedad.</p>
          <Link to="/" className="bg-surface text-brand-primary-700 font-semibold px-6 py-2.5 rounded-xl text-sm inline-block hover:bg-brand-primary-50 transition-colors">
            Ir a la tienda
          </Link>
        </div>
      </div>
