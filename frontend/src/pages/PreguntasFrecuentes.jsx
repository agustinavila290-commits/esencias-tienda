...
export default function PreguntasFrecuentes() {
  return (
    <div className="max-w-2xl mx-auto px-4 py-8 pb-10">
      ...
      <h1 className="font-display text-3xl font-bold text-tierra-800 mb-6">Preguntas frecuentes</h1>

      <div className="space-y-3">
        {PREGUNTAS.map(({ pregunta, respuesta }) => (
          <details key={pregunta} className="bg-white rounded-2xl border border-tierra-100 shadow-sm p-4 group">
            <summary className="font-semibold text-gray-800 cursor-pointer list-none flex items-center justify-between gap-3">
              {pregunta}
              <span className="text-tierra-400 group-open:rotate-45 transition-transform text-xl leading-none">+</span>
            </summary>
            <p className="text-gray-600 text-sm mt-3 leading-relaxed">{respuesta}</p>
          </details>
        ))}
      </div>

      <div className="bg-gradient-to-br from-tierra-700 to-tierra-600 rounded-2xl p-6 text-white text-center mt-8">
        <p className="font-semibold mb-1">¿No encontraste tu respuesta?</p>
        <a
          href={`https://wa.me/${WHATSAPP_NUMBER}`}
          target="_blank" rel="noopener noreferrer"
          className="bg-white text-tierra-700 font-semibold px-6 py-2.5 rounded-xl text-sm inline-block hover:bg-tierra-50 transition-colors mt-3"
        >
